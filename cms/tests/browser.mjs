...
const errors=[];
const base=(process.env.SITE_BASE_URL || 'http://127.0.0.1:8080').replace(/\/$/,'');
page.on('pageerror',error=>errors.push(error.message));
page.on('response',response=>{if((response.url().startsWith(base) || response.url().startsWith('https://cdn.sanity.io/')) && response.status()>=400) errors.push(`${response.status()} ${response.url()}`);});
...
try {
  ...
  await page.goto(`${base}/art.html`,{waitUntil:'networkidle'});
  ...
  await page.goto(`${base}/models/murabella.html`,{waitUntil:'networkidle'});
  ...
  await page.goto(`${base}/commercial.html`,{waitUntil:'networkidle'});
